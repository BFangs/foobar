...
It wouldn't be so bad, except that all the elevator documentation
has been lying in a disorganized pile at the bottom of a filing cabinet for years
...
by a third number named "revision", e.g. 1.1.1, 1.1.2, 1.2.0, and so on.
The number zero can be used as a major for pre-release versions of elevators,
e.g. 0.1, 0.5, 0.9.2, etc (Commander Lambda is careful to always beta test her
new technology, with her loyal henchmen as subjects!).
...
The versions in list l will always contain major numbers, but minor and revision
...
than the others, then these versions must be sorted ascending based on how many
numbers they have, e.g ["1", "1.0", "1.0.0"]. The number of elements in the
...
def answer(l):
    ...
    for seq in l:
        split_seq = seq.split(".")
        int_seq = map(lambda x: int(x), split_seq)
        organized.append(int_seq)
    organized.sort()
    for thing in organized:
        str_thing = map(lambda x: str(x), thing)
        solution.append(".".join(str_thing))
    return solution
